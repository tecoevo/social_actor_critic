...
df_dem = preprocess_df(DataFrame(Arrow.Table("Data/SAC_learning_times_full_demonstrator_choice.arrow")))

df_OG = preprocess_df(DataFrame(Arrow.Table("data/SAC_learning_times_problem_size.arrow")))

df_both = innerjoin(df_dem, df_OG; on = [:n, :x0, :s0, :cov, :ω, :α, :β, :γ, :bias, :rtol, :p, :θ], renamecols = ("_dem" => "_OG"))
...
ylims = (0.2, 5)
xlims = (-0.05, 1.05)
colormap = :cmr_toxic # cmr_toxic, cmr_dusk, cmr_eclipse
yticknums = Any[0.01, 0.1, 0.25, 0.5, 1, 2, 4, 10]
