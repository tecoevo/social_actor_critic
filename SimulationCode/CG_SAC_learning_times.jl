...
ω = rightopenrange(0, 1, 100) 
α = 0.01
β = 0.01
γ = 0.9
bias = [:un, :perf]
rtols = [[0.4]] # must be in descending order and a vector of vector

Tmax = 100_000_000
stability_threshold = 10.
soltol = 1e-6

start_probs_mean = 0.5 # openrange(0, 1, 9)
start_probs_shape = 1.

start_dist_func = [:start_beta_binomial]

filename = "CG_SAC_learning_times_1"
...
@everywhere include("lib/SocialActorCriticEquations/coarse_grained_SAC_equations.jl")
...
@everywhere function learning_time_wrapped(n, x0, s0, cov, ω, α, β, γ, bias, rtols, Tmax, stability_threshold, soltol, start_probs_mean, start_probs_shape, start_dist_func)
    if n == 1
        y0 = [x0, s0]
        return learning_time_1_state_CG_multiple_rtol(y0, ω, α, β, bias, rtols; Tmax, stability_threshold, soltol)
    end
    y0 = create_y0(n, x0, s0, cov)
    start_probs, start_probs_name = eval(start_dist_func)(n, start_probs_mean, start_probs_shape)
    return learning_time_n_state_CG_multiple_rtol(n, y0, ω, α, β, γ, rtols, bias, start_probs; Tmax, stability_threshold, soltol)
end
...
all_pars = product(N, x0, s0, cov_factor, ω, α, β, γ, bias, rtols, Tmax, stability_threshold, soltol, start_probs_mean, start_probs_shape, start_dist_func) 
